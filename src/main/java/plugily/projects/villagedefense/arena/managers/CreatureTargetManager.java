...
//            }
    ...
    public LivingEntity getNearestEntity(Creature creature) {
        ...
        List<Villager> decoys = plugin.getSkillManager() == null ? new ArrayList<>() : plugin.getSkillManager().getDecoys(arena);
        if (!decoys.isEmpty()) {
            return getNearestLivingEntity(location, new ArrayList<>(decoys));
        }
    ...
    private LivingEntity getNearestLivingEntity(Location location, List<? extends LivingEntity> entities) {
        LivingEntity nearestEntity = null;
        for (LivingEntity entity : entities) {
            if (entity == null || entity.isDead() || !entity.getWorld().equals(location.getWorld())) {
                continue;
            }
            if (nearestEntity == null || location.distanceSquared(entity.getLocation()) < location.distanceSquared(nearestEntity.getLocation())) {
                nearestEntity = entity;
            }
        }

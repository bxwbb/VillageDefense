...
public class ShopMenuBedrock extends ShopMenu {
    ...
    public void open(Player player) {
        ...
        for (Merchandise merchandise : ShopManager.merchandises) {
            ...
            cat++;
            if (catIndex < Category.length && cat == Category[catIndex]) {
        ...
        addRemainingCategory(ret, catArray, catIndex);
    ...
    private void addRemainingCategory(Map<String, Object> ret, List<Object> catArray, int catIndex) {
        if (catArray.isEmpty()) {
            return;
        }
        int targetIndex = Math.min(catIndex, CategoryName.length - 1);
        String categoryName = CategoryName[targetIndex];
        if (ret.containsKey(categoryName) && ret.get(categoryName) instanceof List<?> existingList) {
            List<Object> merged = new ArrayList<>(existingList);
            merged.addAll(catArray);
            ret.put(categoryName, merged);
        } else {
            ret.put(categoryName, catArray);
        }
    }
    ...
    public void onNeteasePythonEvent(NeteasePythonEvent event) {
        if (event.getSystemName().equals("main") && event.getNamespace().equals("VillageDefense")) {
            ...
            if (!getShopManager().isCurrentMenu(player, this)) {
                return;
            }
            if (eventName.equals("goodsButtonClick")) {
                int category_index = getInt(data, "category_index", 0);
                int product_slot = getInt(data, "product_slot", -1);
                int level_slot = getInt(data, "level_slot", 0);
                int slot = resolveMerchandiseSlot(category_index, product_slot);
                Merchandise merchandise = ShopManager.getMerchandiseWithSlot(slot);
                if (merchandise == null) {
                    return;
                }
                int level = Math.max(1, Math.min(merchandise.MAX_LEVEL, level_slot + 1));
                getShopManager().playerBuy(player, merchandise, level);
    ...
    private int resolveMerchandiseSlot(int categoryIndex, int productSlot) {
        if (ShopManager.getMerchandiseWithSlot(productSlot) != null) {
            return productSlot;
        }
        return getMerchandiseSlot(categoryIndex, productSlot);
    }

    private int getMerchandiseSlot(int categoryIndex, int productSlot) {
        int slot = productSlot;
        int safeCategoryIndex = Math.max(0, Math.min(categoryIndex, Category.length - 1));
        for (int i = 0; i < safeCategoryIndex; i++) {
            slot += Category[i];
        }
        return slot;
    }
